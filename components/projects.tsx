...
export default function Projects() {
  ...
  return (
    <div className="font-sans">
      <h1 className="text-4xl font-bold mb-4 text-center border-b border-gray-400 shadow-md mx-auto">
        GitHub Projects
      </h1>
      <div className="flex justify-center mx-auto w-full max-w-sm md:max-w-800">
        <input
          type="text"
          className="flex-grow-1 p-0-5 text-lg rounded-lg text-black p-2 mr-3 my-4"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 p-3.5 my-4 mx-auto justify-center items-center text-center self-center -ml-8">
        {filteredProjects.map((project, index) => (
          <div
            className="border-2 border-gray-300 w-full h-full px-5 py-5 m-4 transition ease-in-out duration-200 rounded-lg hover:bg-gray-500 cursor-pointer transform scale-100 shadow-lg hover:shadow-xl"
            onClick={() => (window.location.href = project.html_url)}
            key={project.id || index}
          >
            <h2 className="text-base mb-3 font-medium">{project.name}</h2>
            <p className="text-sm mb-3 text-gray-100">{project.description}</p>
